...
if ( ! class_exists( 'PluginUpdateChecker_2_0' ) ) :
	...
	class PluginUpdateChecker_2_0 {
		...
		public function requestInfo( $queryArgs = array() ) {
			...
			$url = apply_filters( 'puc_request_info_path-' . $this->slug, $url );
		...
		protected function isMuPlugin() {
			...
			if ( $cachedResult === null ) {
				...
				$cachedResult = !empty($muPluginDir) && !empty($pluginPath) && ( strpos( $pluginPath, $muPluginDir ) === 0 );
			}
		...
		/**
		 * Register a callback for modifying url for metadata
		 *
		 * The callback function should take one argument - the url
		 ...
		 * @return void
		 */
		public function addURLPathFilter( $callback ) {
			add_filter( 'puc_request_info_path-' . $this->slug, $callback );
		}
		...
		/**
		 * Initialize the update checker Debug Bar plugin/add-on.
		 */
...
if ( ! class_exists( 'PluginUpdate_2_0' ) ) :
	...
	class PluginUpdate_2_0 {
		...
		public function toStdClass() {
			$object = new StdClass();
			$puc_fields = self::$fields;
			if ( ! empty( $this->slug ) ) {
				$puc_fields = apply_filters( 'puc_retain_fields-' . $this->slug, $puc_fields );
			}
			foreach ( $puc_fields as $field ) {
